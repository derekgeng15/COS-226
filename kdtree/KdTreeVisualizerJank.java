...
public class KdTreeVisualizerJank {
    ...
    private static class PointE {
        ...
        private PointE makeLeftChild(Point2D point, String val) {
            PointE newChild = makeChild(point, this.lb, this.vert);
            this.left = newChild;
            drawSegment(newChild, this.lb, val);
            return newChild;
        }

        // makes right child from `point` and draws it
        private PointE makeRightChild(Point2D point, String val) {
            PointE newChild = makeChild(point, this.rt, this.vert);
            this.right = newChild;
            drawSegment(newChild, this.rt, val);
            return newChild;
        }
    ...
    private static void drawSegment(PointE pointE, RectHV rect, String val) {
        double x = pointE.p.x();
        double y = pointE.p.y();
        ...
        StdDraw.point(x, y);
        StdDraw.textLeft(x + 0.01, y + 0.025, "(" + x + ", " + y + ")" + val);
    }
    ...
    public static void main(String[] args) {
        ...
        KdTreeST<String> kdtree = new KdTreeST<String>();
        ...
        drawSegment(rootpt, rt, kdtree.get(root));
        ...
        while (current < size) {
            ...
            for (int i = current; i < size; i++) {
                ...
                if (parent.lb.contains(p) && !onBoundary(p, parent.lb)) {
                    current = i + 1;
                    queue.enqueue(parent.makeLeftChild(p, kdtree.get(p)));
                    break;
                }
            }
            ...
            for (int i = current; i < size; i++) {
                ...
                if (parent.rt.contains(p) && !onBoundary(p, parent.rt)) {
                    current = i + 1;
                    queue.enqueue(parent.makeRightChild(p, kdtree.get(p)));
                    break;
                }
            }
